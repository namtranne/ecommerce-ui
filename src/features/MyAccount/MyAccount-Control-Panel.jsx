...
const options = [
    ...
    { icon: BsBagHeart, name: "Top Ordered Products" },
    { icon: TbClockSearch, name: "Recently Viewed" },
...
export const MyAccountControlPanel = ({selectedIndex, setSelectedIndex}) => {
    ...
    const calculateIndicatorPosition = (selectedIndex) => {
        const dividerCount = dividerIndices.filter(index => index < selectedIndex).length;
...
const AccountControlOption = ({ optionIcon: OptionIcon, optionName, onClick, isSelected, classNameAdd }) => {
    return (
        <motion.li
            className={twMerge(
                `flex flex-grow pl-10 pr-32 py-3 my-0 items-center w-full space-x-4
                text-zinc-200 text-xl hover:cursor-pointer hover:bg-zinc-700 ${isSelected ? "bg-zinc-700 font-semibold" : ""}`,
                classNameAdd
            )}
            onClick={onClick}
        >
